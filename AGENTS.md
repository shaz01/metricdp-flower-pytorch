...
- `results/<name>/` — every experiment's real output data lives here on `master`, organized to mirror the experiment folder names. One-off shell scripts, and lock/status files don't belong here — only real result data (run JSONs, evaluation summaries, READMEs).
