...
mkdir -p "$rundir" "$evaldir" "$logdir" results-reproduce-step1/tmp
...
PY
  ...
  { flock 9; printf '%s\n' "$line" >> results-reproduce-step1/paper_reproduction_manifest.tsv; printf '| `%s` | %s | %s | %s | %s | %s | %ss | %s |\n' "$name" "$status" "$(echo "$line"|cut -f5)" "$(echo "$line"|cut -f7)" "$(echo "$line"|cut -f8)" "$(echo "$line"|cut -f9)" "$duration" "$note" >> results-reproduce-step1/paper_reproduction_results.md; } 9>results-reproduce-step1/report.lock
}

if valid_artifacts; then
  if ! awk -F '\t' -v n="$name" '$1==n && $2=="PASS" {found=1} END{exit !found}' results-reproduce-step1/paper_reproduction_manifest.tsv 2>/dev/null; then
...
uv run python results-reproduce-step1/evaluate_saved_model.py --model "$model" --run-json "$json" --evaluation-json "$evaluation" --predictions "$predictions" >> "$log" 2>&1
eval_rc=$?
