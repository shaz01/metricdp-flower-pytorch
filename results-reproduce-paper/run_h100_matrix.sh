...
mkdir -p results-reproduce-step1/{runs,logs,evaluations,tmp}
cat > results-reproduce-step1/paper_reproduction_results.md <<'EOF'
...
[[ $(wc -l < results-reproduce-step1/tasks.tsv) -eq 122 ]] || { echo 'Task count is not 122' >&2; exit 2; }

echo "START_UTC=$(date -u +%FT%TZ)" | tee results-reproduce-step1/orchestrator_status.txt
# First pass and two recovery passes. Completed detailed artifacts are validated and skipped.
for pass in 1 2 3; do
  echo "PASS_${pass}_START=$(date -u +%FT%TZ)" | tee -a results-reproduce-step1/orchestrator_status.txt
  xargs -P 2 -n 6 ./results-reproduce-step1/run_one_detailed.sh < results-reproduce-step1/tasks.tsv 2>&1 | tee "results/logs/orchestrator-pass-${pass}.log"
  echo "PASS_${pass}_END=$(date -u +%FT%TZ)" | tee -a results-reproduce-step1/orchestrator_status.txt
done
uv run python results-reproduce-step1/summarize_detailed_results.py
rc=$?
echo "END_UTC=$(date -u +%FT%TZ) SUMMARY_RC=$rc" | tee -a results-reproduce-step1/orchestrator_status.txt
