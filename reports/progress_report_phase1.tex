...
\begin{abstract}
\noindent
This report documents Phase 1 (``Diagnosis'') of an ongoing investigation into why the
...
than at 8. \textbf{Phase 1's goal is to disentangle these two hypotheses} before any mechanism
redesign (Phase 2) is attempted.
...
The net result of this phase to date is not yet ``does the mechanism fail at scale'' --- it is
...
This is, by a wide margin, the strongest and most direct evidence in this phase of work. It does
...
\begin{itemize}[leftmargin=1.4em]
    ...
    /\code{noise\_sweep} result sets that motivated this phase in the first place. Given how
...
\textbf{Bottom line for this reporting period}: Phase 1's stated goal (disentangle round-budget
confound from genuine scaling failure) is \emph{methodologically ready to answer} --- both a flawed
...
\begin{enumerate}[leftmargin=1.4em]
    ...
    \item \textbf{Proceed to the remaining Phase 1 roadmap items} (per-client-count noise-multiplier
...
turned out to be to this phase, item 3 --- forcing CPU for anything that will be cited as a result
--- is the most defensible path for the remainder of Phase 1, reserving MPS for fast exploratory
iteration only.
...
This phase set out to answer a scoping question (is the metric-privacy scaling failure real, or an
artifact of a fixed round budget) and, in the process of building the infrastructure to answer it
rigorously, uncovered a more fundamental problem: the experimental platform itself was not precise
enough to trust the answer. That discovery is itself a legitimate and useful Phase 1 outcome --- it
prevents a false conclusion from being carried into Phase 2's mechanism redesign, where it would be
far more expensive to unwind. The corrected constant-compute control (v2), the confirmed and
documented MPS non-determinism, and the two completed noise-floor checks together define exactly
what further work (per \S\ref{sec:status}) is required before Phase 1's central question can be
