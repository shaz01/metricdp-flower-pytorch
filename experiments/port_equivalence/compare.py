...
ORIGINAL = EXPERIMENT / "original_metricdp_fixed_clipping.py"
...
def compare_seed(seed: int) -> dict[str, float]:
    """Compare both implementations for one seed and return absolute differences."""
    ...
    legacy = run(
        [
            ...
            "--with",
            "scipy==1.14.1",
            str(EXPERIMENT / "legacy_runner.py"),
            str(ORIGINAL),
            str(seed),
        ]
    )
    modern = run([sys.executable, str(EXPERIMENT / "modern_runner.py"), str(seed)])
...
def main() -> None:
    ...
    for seed in SEEDS:
        ...
        within_tolerance = max(differences.values()) <= 1e-15
        all_within_tolerance &= within_tolerance
        print(
            f"seed={seed:<8} "
            f"distance_diff={differences['distance_diff']:.1e} "
            f"stdv_diff={differences['stdv_diff']:.1e} "
            f"output_max_diff={differences['output_max_diff']:.1e} "
