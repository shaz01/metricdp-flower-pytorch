"""Tests for the EuroSAT removal-adjacency CIA stage builder."""

from __future__ import annotations

import pytest

from experiments.cia.scripts import eurosat_remove


def test_build_combos_defaults_to_canonical_client_count() -> None:
    combos = eurosat_remove.build_combos()
    ...
    assert len(combos) == len(eurosat_remove.ADJACENCIES) * len(
        eurosat_remove.PRIVACY_MODES
    )
    assert {c.num_clients for c in in_combos} == {eurosat_remove.CANONICAL_NUM_CLIENTS}
    assert {c.num_clients for c in out_combos} == {
        eurosat_remove.CANONICAL_NUM_CLIENTS - 1
    }
...
def test_build_combos_defaults_to_non_iid_partition() -> None:
    combos = eurosat_remove.build_combos(canonical_num_clients=8)
    assert {c.partition for c in combos} == {"non-iid"}


def test_combos_carry_the_calibrated_noise_multiplier() -> None:
    combos = eurosat_remove.build_combos(canonical_num_clients=8)
    assert {c.noise_multiplier for c in combos} == {0.03710712210729851}
...
def test_fixed_ratio_scales_by_active_client_count() -> None:
    ...
    assert by_adjacency["eurosat-in-remove"].noise_multiplier == pytest.approx(0.008)
    assert by_adjacency["eurosat-out-remove"].noise_multiplier == pytest.approx(0.007)


def test_build_combos_rejects_degenerate_client_counts() -> None:
    with pytest.raises(ValueError):
        eurosat_remove.build_combos(canonical_num_clients=1)
