"""Parametrized Fashion-MNIST (4-class) removal-adjacency CIA stage runner.

Same shape as ``eurosat_remove.py``/``alzheimer_remove.py``: one invocation
...
(``auc_target_search.py``). Uses the repo's existing 4-class Fashion-MNIST
subset (``experiments/reproduce/dataset/fashion_mnist.py``), matching
Alzheimer's class count so the two n=48 non-EuroSAT datasets stay comparable.
"""
...
from experiments.reproduce.dataset.fashion_mnist import FashionMNISTDataModule
...
DEFAULT_OUTPUT_DIR = PROJECT_ROOT / "results" / "cia" / "fashion_mnist_remove"
...
NOISE_MULTIPLIER = 0.03211721152213856  # calibrated 2026-08-17 for this
# model's 168,676 params at n=48. Derivation: target noise-to-signal ratio
...
# update norm (post-clip, post-aggregation, pre-noise) = 1.3740206056121855,
# giving target_noise_multiplier = signal_norm * 48 / (5.0 * sqrt(168676))
# = 0.03211721152213856.
...
def create_in_remove(config: Mapping[str, Any]) -> PartitionViewDataModule:
    return in_remove(
        FashionMNISTDataModule(cache_dir=_cache_dir(config)),
...
def build_combos(
    *,
    ...
    return [
        Combo(
            name_prefix=f"fashion-mnist-{adjacency}",
...
def run_stage(
    *,
    ...
    return run_attack(
        ...
        force=force,
        start_message=(
            f"Fashion-MNIST stage ({privacy}, {partition}, seed={seed}, "
...
def main() -> None:
    ...
    run_attack(
        ...
        force=args.force,
        start_message=(
            f"Fashion-MNIST removal CIA chunk ({args.privacy}, clients={args.clients}, "
